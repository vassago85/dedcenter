...
<head>
    ...
    <title>{{ __('Page not found') }} — DeadCenter</title>
...
<body class="min-h-screen bg-app text-primary antialiased">
    <div class="flex min-h-screen items-center justify-center px-4 py-12">
        <div class="w-full max-w-md rounded-2xl border border-border bg-surface p-8 text-center shadow-xl">
            <div class="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-full bg-sky-500/10 text-sky-400">
                <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/>
                </svg>
            </div>
            <p class="text-xs font-semibold uppercase tracking-wider text-muted">Error 404</p>
            <h1 class="mt-1 text-2xl font-bold text-primary">{{ __('Page not found') }}</h1>
            <p class="mt-3 text-sm text-secondary">
                {{ $exception->getMessage() ?: __('The page you were looking for doesn\'t exist or has moved.') }}
            </p>
