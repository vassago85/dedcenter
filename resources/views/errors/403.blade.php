...
<head>
    ...
    <title>{{ __('Forbidden') }} — DeadCenter</title>
...
<body class="min-h-screen bg-app text-primary antialiased">
    <div class="flex min-h-screen items-center justify-center px-4 py-12">
        <div class="w-full max-w-md rounded-2xl border border-border bg-surface p-8 text-center shadow-xl">
            <div class="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-full bg-amber-500/10 text-amber-400">
                <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <path d="M12 9v4"/><path d="M12 17h.01"/><path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0Z"/>
                </svg>
            </div>
            <p class="text-xs font-semibold uppercase tracking-wider text-muted">Error 403</p>
            <h1 class="mt-1 text-2xl font-bold text-primary">{{ __('Not available') }}</h1>
            <p class="mt-3 text-sm text-secondary">
                {{ $exception->getMessage() ?: __('You don\'t have access to this page right now.') }}
            </p>
