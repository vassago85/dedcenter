...
/*
|--------------------------------------------------------------------------
| Match Control Center → Scoring tab (platform admin context).
|--------------------------------------------------------------------------
| Mirrors the org scoring page but routes through the admin URL space.
| Authorisation is provided by the `admin` middleware on the route — no
| extra in-component check needed (platform admins implicitly outrank
            ...
            <div class="mb-4 flex items-start gap-3 rounded-xl border border-amber-500/30 bg-amber-500/8 p-4">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <p class="mt-0.5 text-xs text-amber-100/75">
                        The match is in <span class="font-semibold">{{ $status->label() }}</span>. The scoring app stays locked until you advance the lifecycle to <span class="font-semibold">Ready</span> or <span class="font-semibold">Active</span>.
                    </p>
                </div>
            </div>
        @elseif($isCompleted)
                ...
                <div class="min-w-0 flex-1 sm:flex sm:items-start sm:justify-between sm:gap-4">
                    <div class="min-w-0">
                        ...
                        <p class="mt-0.5 text-xs text-zinc-400">
                            New scores can't be captured until you reopen the match. Per-squad corrections below stay available.
                        </p>
                    </div>
                    <button
                        type="button"
            ...
            <section class="mt-4 rounded-2xl border border-border bg-surface p-5 sm:p-6">
                <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                        ...
                        <p class="mt-0.5 text-xs text-muted">Edit the hit/miss matrix for a squad without re-opening the match. Every change writes an audit entry.</p>
                    </div>
                </div>
                ...
                @else
                    <div class="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                        @foreach($squads as $squad)
                            <a
                                href="{{ route('admin.matches.squad-correction', [$match, $squad]) }}"
                                wire:navigate
        ...
        @elseif(! $isStandardScoring && ($isActive || $isCompleted))
            <section class="mt-4 rounded-2xl border border-border bg-surface p-5 sm:p-6">
                ...
                <p class="mt-1.5 text-xs text-muted">
                    {{ strtoupper($match->scoring_type) }} scoring uses per-shooter corrections directly inside the scoring app.
                </p>
            </section>
        @endif
