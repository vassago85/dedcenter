...
<div class="space-y-6">
    <div>
        ...
        <p class="mt-1 text-sm text-slate-400">{{ $organization->name }} — Review and approve member registrations.</p>
    </div>
    ...
    <div class="rounded-xl border border-slate-700 bg-slate-800 overflow-hidden">
        @if($registrations->isEmpty())
            <div class="px-6 py-12 text-center"><p class="text-slate-400">No registrations matching this filter.</p></div>
        @else
                            ...
                            <tr class="hover:bg-slate-700/30 transition-colors" wire:key="reg-{{ $reg->id }}">
                                ...
                                <td class="px-6 py-3">
                                    @if($reg->proof_of_payment_path)
                                        <a href="{{ Storage::url($reg->proof_of_payment_path) }}" target="_blank" class="text-red-400 hover:text-red-300 text-xs font-medium">View POP</a>
                                    @else
                                ...
                                <td class="px-6 py-3 text-right">
                                    @if($reg->payment_status === 'proof_submitted')
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            <flux:button size="sm" variant="ghost" class="!text-red-400 hover:!text-red-300" wire:click="reject({{ $reg->id }})" wire:confirm="Reject this registration?">Reject</flux:button>
                                        </div>
                                    @endif
                                </td>
                            </tr>
