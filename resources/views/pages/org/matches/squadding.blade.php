...
new #[Layout('components.layouts.app')]
    ...
    /**
     * Swap this shooter with the immediate neighbour above them (by sort_order)
     * within the same squad. No-op at the top of the list.
     */
    ...
    /**
     * Swap this shooter with the immediate neighbour below them (by sort_order)
     * within the same squad. No-op at the bottom of the list.
     */
    ...
    {
        $this->match->shooters()->where('status', 'active')->update(['status' => 'active']);
    ...
    public function addWalkin(): void
    {
        ...
        if ($this->walkinUserId) {
            ...
            if ($existing) {
                Flux::toast("{$this->walkinName} is already in this match.", variant: 'danger');
                return;
            }
        }
...
<div class="space-y-6 max-w-6xl" x-data="{ tab: @entangle('activeTab') }">
    ...
    <div class="flex gap-1 border-b border-border">
        <button @click="tab = 'squads'"
                ...
                class="border-b-2 px-4 py-2.5 text-sm font-medium transition-colors min-h-[44px]">
            Squads
        </button>
        ...
        @if($isTeamEvent)
            <button @click="tab = 'teams'"
                    :class="tab === 'teams' ? 'border-accent text-primary' : 'border-transparent text-muted hover:text-secondary'"
                    class="border-b-2 px-4 py-2.5 text-sm font-medium transition-colors min-h-[44px]">
                Teams
            </button>
        @endif
    ...
    <div x-show="tab === 'squads'" x-cloak class="space-y-5">
        ...
        {{-- Concurrent group legend — only relevant to formats that run
             multiple relays in parallel (Royal Flush, Steel Challenge).
             PRS runs one squad at a time so the legend just adds noise. --}}
            ...
            <div class="space-y-3">
                ...
                @foreach($squads as $squad)
                    @php
                        ...
                        $squadActive = $squad->shooters->where('status', 'active')->count();
                        $squadNoShow = $squad->shooters->where('status', 'no_show')->count();
                    @endphp
                    <div class="rounded-xl border border-border bg-surface overflow-hidden" wire:key="sq-{{ $squad->id }}">
                        <div class="flex items-center justify-between border-b border-border px-4 py-2.5">
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                @if($squadNoShow > 0)
                                    <span class="rounded px-1.5 py-0.5 text-[10px] bg-zinc-500/10 text-zinc-400 border border-zinc-500/20">{{ $squadNoShow }} no-show</span>
                                @endif
                            </div>
                            <div class="flex items-center gap-2">
                                <a href="{{ route('org.matches.squad-correction', [$organization, $match, $squad]) }}"
                                    ...
                                    <table class="w-full text-sm">
                                        ...
                                        <tbody class="divide-y divide-border/30">
                                            @foreach($squad->shooters as $shooter)
                                                <tr wire:key="sh-{{ $shooter->id }}" class="{{ $shooter->isNoShow() ? 'opacity-50' : '' }} {{ $shooter->isDq() ? 'opacity-40 line-through' : '' }}">
                                                    ...
                                                    @endif
                                                    <td class="px-2 py-1.5">
                                                        <span class="inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-medium {{ $shooter->statusBadgeClasses() }}">
                                                            {{ $shooter->statusLabel() }}
                                                        </span>
                                                    </td>
                                                    <td class="px-2 py-1.5">
    ...
    <div x-show="tab === 'matchday'" x-cloak class="space-y-5">
        ...
        {{-- Full shooter roster with status management --}}
        @if($totalShooters > 0)
                                ...
                                @foreach($squad->shooters->sortBy('name') as $shooter)
                                    <tr wire:key="md-{{ $shooter->id }}" class="{{ $shooter->isNoShow() ? 'opacity-50 bg-zinc-900/20' : '' }} {{ $shooter->isDq() ? 'opacity-40 bg-red-900/10' : '' }}">
                                        ...
                                        <td class="px-4 py-2.5 text-xs text-muted">
                                            @if(in_array($squad->name, ['Default', 'Unassigned']))
                                        ...
                                        <td class="px-4 py-2.5">
                                            <span class="inline-flex items-center rounded-full border px-2.5 py-0.5 text-[10px] font-medium {{ $shooter->statusBadgeClasses() }}">
                                                {{ $shooter->statusLabel() }}
                                            </span>
                                        </td>
                                        <td class="px-4 py-2.5 text-right">
                                            <div class="flex items-center justify-end gap-1">
                                                @if($shooter->isActive())
                                                    <button wire:click="markNoShow({{ $shooter->id }})"
                                                            class="rounded px-2 py-1 text-[10px] font-medium text-zinc-400 hover:bg-zinc-500/10 transition-colors min-h-[32px]"
                                                            title="Mark as no-show">No-Show</button>
                                                    <button wire:click="markWithdrawn({{ $shooter->id }})"
                                                            class="rounded px-2 py-1 text-[10px] font-medium text-amber-400 hover:bg-amber-500/10 transition-colors min-h-[32px]"
                                                            title="Mark as withdrawn">Withdrawn</button>
                                                @elseif($shooter->isNoShow() || $shooter->isWithdrawn())
                                                    <button wire:click="markPresent({{ $shooter->id }})"
                                                            class="rounded px-2 py-1 text-[10px] font-medium text-green-400 hover:bg-green-500/10 transition-colors min-h-[32px]"
                                                            title="Reinstate as active">Reinstate</button>
                                                @elseif($shooter->isDq())
    ...
    <div x-show="tab === 'walkin'" x-cloak class="space-y-5">
        ...
        <div class="rounded-xl border border-border bg-surface p-6 space-y-5">
            <div>
                ...
                <p class="text-sm text-muted mt-1">Someone showed up on match day without pre-registering? Add them here. If they have a DeadCenter account, search for them to link their profile — their scores will then appear on their record.</p>
            </div>
            ...
            <div class="space-y-2">
                ...
                @else
                    <div class="relative" x-data="{ focused: false }">
                        <input type="text"
                               wire:model.live.debounce.300ms="walkinSearch"
                               @focus="focused = true"
                            ...
                            <div x-show="focused" x-transition x-cloak class="absolute left-0 right-0 z-20 mt-1 max-h-60 overflow-y-auto rounded-lg border border-border bg-surface-2 shadow-xl">
                                @foreach($userResults as $u)
                                    <button wire:click="selectWalkinUser({{ $u['id'] }})" @click="focused = false"
                                        ...
                                        <div class="flex h-8 w-8 items-center justify-center rounded-full bg-surface text-xs font-bold text-primary shrink-0">
                        ...
                        @elseif(strlen($walkinSearch) >= 2)
                            <div x-show="focused" x-cloak class="absolute left-0 right-0 z-20 mt-1 rounded-lg border border-border bg-surface-2 px-4 py-3 shadow-xl">
                                <p class="text-sm text-muted">No matching users found. You can still add them as an unlinked walk-in below.</p>
                            </div>
                        @endif
                    </div>
                @endif
        ...
        {{-- Quick tips --}}
        <div class="rounded-xl border border-blue-600/20 bg-blue-900/10 p-5 space-y-3">
    ...
    {{-- ═══════════════════════════════════════════════
         TAB: Teams (only if team event)
         ═══════════════════════════════════════════════ --}}
    @if($isTeamEvent)
        <div x-show="tab === 'teams'" x-cloak class="space-y-5">
            <div class="rounded-xl border border-indigo-500/30 bg-surface p-6 space-y-4">
                <div class="flex items-center justify-between">
                    ...
                    <flux:button wire:click="autoTeam" variant="primary" class="!bg-indigo-600 hover:!bg-indigo-700 min-h-[44px]" size="sm"
                                    ...
                                    <ul class="space-y-1">
                                        @foreach($team->shooters as $ts)
                                            <li class="flex items-center justify-between text-sm text-secondary">
                                                <span>{{ $ts->name }}</span>
                                                <button wire:click="removeFromTeam({{ $ts->id }})" class="text-xs text-accent/60 hover:text-accent min-h-[32px]">&times;</button>
                                            </li>
                                        @endforeach
                                    </ul>
                                @else
                            ...
                            @foreach($unteamedShooters as $us)
                                <div class="flex items-center gap-1 rounded-lg border border-border bg-surface px-3 py-1.5" wire:key="ut-{{ $us->id }}" x-data="{ tOpen: false }" @click.away="tOpen = false">
                                    ...
                                    <div class="relative ml-1">
                                        <button @click="tOpen = !tOpen" class="text-xs text-muted hover:text-secondary min-h-[32px]">&#8644;</button>
                                        <div x-show="tOpen" x-transition x-cloak class="absolute left-0 z-10 mt-1 w-40 rounded-lg border border-border bg-surface-2 py-1 shadow-lg">
                                            @foreach($teams as $tm)
                                                @if(!$tm->isFull())
                                                    <button wire:click="assignToTeam({{ $us->id }}, {{ $tm->id }})" @click="tOpen = false"
                                                            class="block w-full px-3 py-1.5 text-left text-xs text-secondary hover:bg-surface hover:text-white transition-colors min-h-[36px]">{{ $tm->name }}</button>
                                                @endif
                                            @endforeach
                                        </div>
                                    </div>
