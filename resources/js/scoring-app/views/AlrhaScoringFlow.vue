...
const classLabel = computed(() => {
    const c = match.value?.alrha_class;
...
const shooters = computed(() => activeSquad.value?.shooters ?? []);
...
const blockTargets = computed(() => {
    ...
    return stages.value.flatMap(stage => (stage.targets ?? []))
        .filter(target => (target.alrha_block ?? '') === key);
});
