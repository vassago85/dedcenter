...
class ElrDemoMatchSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // Stage 1 — Ladder
        $stage1 = $match->elrStages()->create([
            'label' => 'Stage 1 — Ladder',
            'stage_type' => 'ladder',
        ...
        foreach ($ladderTargets as $i => $t) {
            $stage1->targets()->create([...$t, 'max_shots' => 3, 'sort_order' => $i + 1]);
        }

        // Stage 2 — Static
        $stage2 = $match->elrStages()->create([
            'label' => 'Stage 2 — Static',
            'stage_type' => 'static',
            'elr_scoring_profile_id' => $profile->id,
            'sort_order' => 2,
        ]);
        ...
        foreach ($staticTargets as $i => $t) {
            $stage2->targets()->create([...$t, 'max_shots' => 3, 'sort_order' => $i + 1]);
        }
        ...
        $squads = [
            Squad::create(['match_id' => $match->id, 'name' => 'Squad A', 'sort_order' => 1]),
            Squad::create(['match_id' => $match->id, 'name' => 'Squad B', 'sort_order' => 2]),
        ];

        foreach ($names as $i => $name) {
            $squad = $squads[$i < 5 ? 0 : 1];
            $squad->shooters()->create([
                'name' => $name,
